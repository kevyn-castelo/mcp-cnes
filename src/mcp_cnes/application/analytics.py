...
class UseBatch:
    ...
    def execute(self, batch_id: str) -> None:
        if not batch_id.strip():
            raise DomainValidationError("lote_id não pode ser vazio")
...
class TimeSeries:
    ...
    def execute(
        ...
        if start > end:
            raise DomainValidationError("de não pode ser posterior a ate")
        if not key.strip():
            raise DomainValidationError("chave não pode ser vazia")
...
class DiffBatches:
    ...
    def execute(self, batch_a: str, batch_b: str) -> dict[str, Any]:
        if batch_a == batch_b:
            raise DomainValidationError("lote_a e lote_b devem ser diferentes")
...
def _validate_filters(filters: Mapping[str, Any]) -> None:
    ...
    if extras:
        raise DomainValidationError(f"Filtros não suportados: {', '.join(sorted(extras))}")
    uf = filters.get("uf")
    ...
    if convenio is not None and not isinstance(convenio, bool):
        raise DomainValidationError("convenio_sus deve ser booleano")
    ...
    if cnes_list is not None and (
        ...
        raise DomainValidationError(
            "cnes_list deve conter de 1 a 500 códigos CNES válidos"
        )
